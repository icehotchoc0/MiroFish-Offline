...
def generate_report():
    ...
    GET /api/report/generate/status로 진행 상황을 확인하세요.
    ...
    try:
        ...
        # 프론트엔드에 즉시 반환하기 위해 report_id를 미리 생성
        import uuid
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "보고서 생성 태스크가 시작되었습니다. /api/report/generate/status를 통해 진행 상황을 확인하세요",
...
def chat_with_report_agent():
    ...
    Report Agent는 대화 중 자율적으로 검색 도구를 호출하여 질문에 답변합니다
...
def get_report_sections(report_id: str):
    ...
    프론트엔드에서 이 인터페이스를 폴링하여 생성된 섹션 내용을 가져올 수 있으며, 전체 보고서 완료를 기다릴 필요 없습니다
...
def check_report_status(simulation_id: str):
    ...
    프론트엔드에서 Interview 기능 잠금 해제 여부 판단에 사용
...
def get_agent_log(report_id: str):
    ...
    보고서 생성 과정의 각 단계 동작을 실시간으로 가져옵니다:
    - 보고서 시작, 계획 시작/완료
    - 각 섹션의 시작, 도구 호출, LLM 응답, 완료
    - 보고서 완료 또는 실패

    Query 매개변수:
        from_line: 몇 번째 줄부터 읽을지 (선택, 기본값 0, 증분 조회용)
...
def search_graph_tool():
    ...
    try:
        ...
        if not graph_id or not query:
            return jsonify({
                "success": False,
                "error": "graph_id와 query를 입력해 주세요"
            }), 400
