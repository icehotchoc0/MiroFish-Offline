...
def create_simulation():
    ...
    참고: max_rounds 등의 매개변수는 LLM이 자동 생성하므로 수동 설정이 필요 없습니다
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "프로젝트에 아직 그래프가 구축되지 않았습니다. 먼저 /api/graph/build를 호출해 주세요"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        if status in prepared_statuses and config_generated:
            ...
            # 상태가 preparing이지만 파일이 완료된 경우, 자동으로 상태를 ready로 업데이트
            if status == "preparing":
                try:
...
def prepare_simulation():
    """
    시뮬레이션 환경 준비 (비동기 태스크, LLM이 모든 매개변수를 자동 생성)

    시간이 소요되는 작업으로, 인터페이스는 즉시 task_id를 반환합니다.
    GET /api/simulation/prepare/status로 진행 상황을 확인하세요.
    ...
    4. LLM 자동 시뮬레이션 설정 생성 (재시도 메커니즘 포함)
    ...
    반환:
        {
            ...
            "data": {
                ...
                "message": "준비 태스크 시작됨|완료된 준비 작업 존재",
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "완료된 준비 작업이 있습니다. 중복 생성이 필요 없습니다",
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "준비 태스크가 시작되었습니다. /api/simulation/prepare/status를 통해 진행 상황을 확인하세요",
...
def get_prepare_status():
    ...
    try:
        ...
        # simulation_id가 제공된 경우, 준비 완료 여부 먼저 확인
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task_id:
            ...
            return jsonify({
                "success": False,
                "error": "task_id 또는 simulation_id를 입력해 주세요"
            }), 400
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    reports 디렉토리를 순회하여 simulation_id와 일치하는 report 찾기,
    여러 개인 경우 최신 것 반환 (created_at 기준 정렬)
    ...
    # __file__은 app/api/simulation.py이며, backend/까지 두 단계 상위로 이동 필요
    ...
    try:
        ...
        logger.warning(f"simulation {simulation_id}의 report 조회 실패: {e}")
        return None
...
def get_simulation_history():
    ...
    홈페이지 히스토리 프로젝트 표시용, 프로젝트 이름/설명 등 풍부한 정보를 포함한 시뮬레이션 목록 반환
            ...
            "data": [
                {
                    ...
                    "project_id": "proj_xxxx",
                    "project_name": "여론 분석",
                    "simulation_requirement": "만약 대학교에서 발표하면...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # 사용자 설정 total_rounds 사용, 없으면 추천 라운드 수 사용
            ...
            # 연관 report_id 가져오기 (해당 simulation의 최신 report 조회)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    시뮬레이션의 Agent Profile 실시간 조회 (생성 과정에서 진행 상황 실시간 확인용)
    
    /profiles 인터페이스와의 차이점:
    - 파일을 직접 읽기, SimulationManager를 거치지 않음
    - 생성 과정에서의 실시간 확인에 적합
...
def get_simulation_config_realtime(simulation_id: str):
    """
    시뮬레이션 설정 실시간 조회 (생성 과정에서 진행 상황 실시간 확인용)
    
    /config 인터페이스와의 차이점:
    - 파일을 직접 읽기, SimulationManager를 거치지 않음
    - 생성 과정에서의 실시간 확인에 적합
    - 추가 메타데이터 반환 (파일 수정 시간, 생성 중 여부 등)
    - 설정이 아직 완전히 생성되지 않았어도 부분 정보 반환 가능
...
def generate_profiles():
    """
    그래프에서 직접 OASIS Agent Profile 생성 (시뮬레이션 생성 없음)
...
def start_simulation():
    ...
    요청 (JSON):
        {
            ...
            "max_rounds": 100,                     // 선택: 최대 시뮬레이션 라운드 수, 너무 긴 시뮬레이션 절단용
            "enable_graph_memory_update": false,   // 선택: Agent 활동을 Zep 그래프 메모리에 동적 업데이트 여부
    ...
    enable_graph_memory_update 관련:
        - 활성화 시 시뮬레이션의 모든 Agent 활동 (게시, 댓글, 좋아요 등)이 Zep 그래프에 실시간 업데이트
        - 그래프가 시뮬레이션 과정을 "기억"하도록 하여 후속 분석이나 AI 대화에 활용
        - 시뮬레이션과 연관된 프로젝트에 유효한 graph_id가 필요
        - 배치 업데이트 메커니즘 채용, API 호출 횟수 감소
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"유효하지 않은 플랫폼 유형: {platform}, 가능한 값: twitter/reddit/parallel"
            }), 400
        ...
        # 스마트 상태 처리: 준비 작업이 완료된 경우 재시작 허용
        if state.status != SimulationStatus.READY:
            # 준비 작업 완료 여부 확인
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            ...
            if is_prepared:
                ...
                # 프로세스가 존재하지 않거나 종료됨, 상태를 ready로 초기화
                logger.info(f"시뮬레이션 {simulation_id} 준비 작업 완료, 상태를 ready로 초기화 (이전 상태: {state.status.value})")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "그래프 메모리 업데이트 활성화에는 유효한 graph_id가 필요합니다. 프로젝트에 그래프가 구축되었는지 확인해 주세요"
                }), 400
...
def get_simulation_actions(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"동작 기록 조회 실패: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_comments(simulation_id: str):
    """
    시뮬레이션의 댓글 조회 (Reddit만)
    
    Query 매개변수:
        post_id: 게시글 ID 필터링 (선택)
        limit: 반환 수량
        offset: 오프셋
    """
    try:
...
def interview_agent():
    ...
    참고: 이 기능은 시뮬레이션 환경이 실행 상태여야 합니다 (시뮬레이션 루프 완료 후 명령 대기 모드 진입)
    ...
    요청 (JSON):
        {
            ...
            "timeout": 60                      // 선택，타임아웃 시간 (초), 기본값 60
        }
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "시뮬레이션 환경이 실행 중이 아니거나 종료되었습니다. 시뮬레이션이 완료되어 명령 대기 모드에 진입했는지 확인해 주세요."
            }), 400
    ...
    요청 (JSON):
        {
            ...
            "platform": "reddit",              // 선택，기본 플랫폼 (각 항목의 platform으로 재정의됨)
                                               // 미지정 시: 이중 플랫폼 시뮬레이션에서 각 Agent를 두 플랫폼에서 동시 인터뷰
            "timeout": 120                     // 선택，타임아웃 시간 (초), 기본값 120
        }
...
def interview_all_agents():
    ...
    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",            // 필수, 시뮬레이션 ID
            "prompt": "이 일 전체에 대해 어떻게 생각하시나요?",  // 필수，인터뷰 질문 (모든 Agent에 동일 질문 사용)
            "platform": "reddit",                   // 선택，플랫폼 지정 (twitter/reddit)
                                                    // 미지정 시: 이중 플랫폼 시뮬레이션에서 각 Agent를 두 플랫폼에서 동시 인터뷰
            "timeout": 180                          // 선택，타임아웃 시간 (초), 기본값 180
        }
...
def get_interview_history():
    ...
    요청 (JSON):
        {
            ...
            "platform": "reddit",          // 선택，플랫폼 유형 (reddit/twitter)
                                           // 미지정 시 두 플랫폼의 모든 기록 반환
            "agent_id": 0,                 // 선택，해당 Agent의 인터뷰 기록만 조회
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # 미지정 시 두 플랫폼의 기록 반환
        agent_id = data.get('agent_id')
...
def close_simulation_env():
    ...
    시뮬레이션에 환경 종료 명령을 전송하여 명령 대기 모드에서 우아하게 종료합니다.

    참고: /stop 인터페이스와 다릅니다. /stop은 프로세스를 강제 종료하지만,
    이 인터페이스는 시뮬레이션이 우아하게 환경을 종료하고 나갈 수 있게 합니다.
    
    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",  // 필수, 시뮬레이션 ID
            "timeout": 30                  // 선택，타임아웃 시간 (초), 기본값 30
        }
