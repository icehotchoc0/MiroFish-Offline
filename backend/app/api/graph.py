...
def reset_project(project_id: str):
    ...
    return jsonify({
        "success": True,
        "message": f"프로젝트 초기화됨: {project_id}",
        "data": project.to_dict()
    })
...
def generate_ontology():
    """
    인터페이스 1: 파일 업로드, 분석하여 온톨로지 정의 생성
    ...
    try:
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "시뮬레이션 요구사항 설명을 입력해 주세요 (simulation_requirement)"
            }), 400
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "문서를 처리하지 못했습니다. 파일 형식을 확인해 주세요"
            }), 400
        ...
        # 온톨로지 생성
        logger.info("LLM 호출하여 온톨로지 정의 생성...")
        ...
        logger.info(f"온톨로지 생성 완료: {entity_count}개 엔티티 유형, {edge_count}개 관계 유형")
...
def build_graph():
    """
    인터페이스 2: project_id 기반 그래프 구축

    요청 (JSON):
        {
            "project_id": "proj_xxxx",  // 필수, 인터페이스 1에서 반환
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "프로젝트에 아직 온톨로지가 생성되지 않았습니다. 먼저 /ontology/generate를 호출해 주세요"
            }), 400
        ...
        # 요청 컨텍스트에서 storage 가져오기 (백그라운드 스레드에서는 current_app 접근 불가)
        ...
        def build_task():
            ...
            try:
                ...
                # 그래프 구축 서비스 생성 (storage는 외부 클로저에서 전달)
                ...
                # Neo4j 처리는 동기식이므로 대기 불필요
                task_manager.update_task(
                    task_id,
                    message="텍스트 처리 완료, 그래프 데이터 생성...",
                    progress=90
                )
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "그래프 구축 태스크가 시작되었습니다. /task/{task_id}를 통해 진행 상황을 확인하세요"
            }
        })
