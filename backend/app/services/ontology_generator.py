...
**중요: 유효한 JSON 형식의 데이터만 출력해야 하며, 다른 내용은 출력하지 마세요.**

## 핵심 과제 배경

우리는 **소셜 미디어 여론 시뮬레이션 시스템**을 구축하고 있습니다. 이 시스템에서:
- 각 엔티티는 소셜 미디어에서 발언하고, 상호작용하고, 정보를 전파할 수 있는 "계정" 또는 "주체"입니다
- 엔티티 간에는 서로 영향을 주고, 공유하고, 댓글을 달고, 응답합니다
- 여론 사건에서 각 측의 반응과 정보 전파 경로를 시뮬레이션해야 합니다

따라서, **엔티티는 현실에서 실제로 존재하며, 소셜 미디어에서 발언하고 상호작용할 수 있는 주체여야 합니다**:

**가능한 엔티티**:
- 특정 개인 (공인, 당사자, 오피니언 리더, 전문가/학자, 일반인)
- 회사, 기업 (공식 계정 포함)
- 조직/기관 (대학교, 협회, NGO, 노동조합 등)
- 정부 부처, 규제 기관
- 미디어 기관 (신문사, 방송국, 1인 미디어, 웹사이트)
- 소셜 미디어 플랫폼 자체
- 특정 그룹 대표 (예: 동문회, 팬클럽, 권리옹호 단체 등)

**불가능한 엔티티**:
- 추상적 개념 (예: "여론", "감정", "트렌드")
- 주제/화제 (예: "학문적 진실성", "교육 개혁")
- 관점/태도 (예: "지지측", "반대측")

## 출력 형식

JSON 형식으로 출력하며, 다음 구조를 포함합니다:
...
**수량 요구사항: 정확히 10개의 엔티티 유형이어야 합니다**

**계층 구조 요구사항 (구체적 유형과 포괄적 유형을 모두 포함해야 함)**:

10개의 엔티티 유형에는 다음 계층이 포함되어야 합니다:

A. **포괄적 유형 (반드시 포함, 목록 마지막 2개에 배치)**:
   - `Person`: 모든 자연인 개인의 포괄적 유형. 다른 더 구체적인 인물 유형에 속하지 않을 때 이 유형으로 분류합니다.
   - `Organization`: 모든 조직/기관의 포괄적 유형. 다른 더 구체적인 조직 유형에 속하지 않을 때 이 유형으로 분류합니다.

B. **구체적 유형 (8개, 텍스트 내용에 따라 설계)**:
   - 텍스트에 등장하는 주요 역할에 대해 더 구체적인 유형을 설계합니다
   - 예: 텍스트가 학술 사건과 관련된 경우, `Student`, `Professor`, `University` 등이 가능합니다
   - 예: 텍스트가 비즈니스 사건과 관련된 경우, `Company`, `CEO`, `Employee` 등이 가능합니다

**포괄적 유형이 필요한 이유**:
- 텍스트에는 "초중고 교사", "지나가는 사람", "어떤 네티즌" 등 다양한 인물이 등장합니다
- 전문 유형이 매칭되지 않으면 `Person`으로 분류되어야 합니다
- 마찬가지로, 소규모 조직, 임시 단체 등은 `Organization`으로 분류되어야 합니다

**구체적 유형의 설계 원칙**:
- 텍스트에서 자주 등장하거나 핵심적인 역할 유형을 식별합니다
...
- 수량: 6-10개
- 관계는 소셜 미디어 상호작용에서의 실제 연결을 반영해야 합니다
- 관계의 source_targets가 정의한 엔티티 유형을 포괄하는지 확인합니다
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # 텍스트가 5만 자를 초과하면 잘라냄 (LLM에 전달하는 내용만 영향, 그래프 구축에는 영향 없음)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(원문 총 {original_length}자, 온톨로지 분석을 위해 앞의 {self.MAX_TEXT_LENGTH_FOR_LLM}자를 추출했습니다)..."
...
"""
        ...
        message += """
위의 내용을 바탕으로, 사회 여론 시뮬레이션에 적합한 엔티티 유형과 관계 유형을 설계하세요.

**반드시 준수해야 할 규칙**:
1. 정확히 10개의 엔티티 유형을 출력해야 합니다
2. 마지막 2개는 포괄적 유형이어야 합니다: Person (개인 포괄) 및 Organization (조직 포괄)
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            # 추가 시 10개를 초과하면 기존 유형을 일부 제거해야 함
            if current_count + needed_slots > MAX_ENTITY_TYPES:
                # 제거해야 할 개수 계산
                to_remove = current_count + needed_slots - MAX_ENTITY_TYPES
                # 끝에서부터 제거 (앞의 더 중요한 구체적 유형은 유지)
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        """
        [DEPRECATED] 온톨로지 정의를 Zep-format Pydantic 코드로 변환.
