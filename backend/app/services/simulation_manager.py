"""
OASIS 시뮬레이션 관리자
Twitter와 Reddit 듀얼 플랫폼 병렬 시뮬레이션 관리
사전 설정 스크립트 + LLM 지능형 설정 파라미터 생성
"""
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        3. LLM을 사용하여 시뮬레이션 설정 파라미터 지능형 생성 (시간, 활성도, 발언 빈도 등)
        ...
        try:
            ...
            if progress_callback:
                progress_callback("reading", 0, "그래프에 연결 중...")
            ...
            # graph_id를 전달하여 그래프 검색 기능을 활성화하고 더 풍부한 컨텍스트 획득
            ...
            # Reddit은 생성 과정에서 이미 실시간 저장됨, 여기서 다시 저장하여 완전성 보장
            if progress_callback:
                progress_callback(
            ...
            # 참고: 실행 스크립트는 backend/scripts/ 디렉토리에 유지, 시뮬레이션 디렉토리로 복사하지 않음
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                ...
                f"   - 듀얼 플랫폼 병렬 실행: python {scripts_dir}/run_parallel_simulation.py --config {config_path}"
            )
        }
