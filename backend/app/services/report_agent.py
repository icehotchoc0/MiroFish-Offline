"""
Report Agent 서비스
ReACT 패턴을 사용하여 시뮬레이션 보고서 생성 (via GraphStorage / Neo4j)
...
【광범위 검색 - 전체 조감도 가져오기】
이 도구는 시뮬레이션 결과의 전체 조감도를 가져오는 데 사용되며, 특히 이벤트 변화 과정을 이해하는 데 적합합니다. 다음을 수행합니다:
1. 모든 관련 노드와 관계를 가져옴
2. 현재 유효한 사실과 과거/만료된 사실을 구분
3. 여론이 어떻게 변화했는지 파악하는 데 도움

【사용 시나리오】
- 이벤트의 전체 발전 과정을 파악해야 할 때
...
【심층 인터뷰 - 실제 에이전트 인터뷰 (듀얼 플랫폼)】
OASIS 시뮬레이션 환경의 인터뷰 API를 호출하여 실행 중인 시뮬레이션 에이전트를 실제로 인터뷰합니다!
이것은 LLM 시뮬레이션이 아니라, 실제 인터뷰 인터페이스를 호출하여 시뮬레이션 에이전트의 원본 답변을 가져옵니다.
기본적으로 Twitter와 Reddit 두 플랫폼에서 동시에 인터뷰하여 더 포괄적인 관점을 가져옵니다.

기능 흐름:
1. 자동으로 페르소나 파일을 읽어 모든 시뮬레이션 에이전트를 파악
2. 인터뷰 주제와 가장 관련 있는 에이전트를 지능적으로 선택 (예: 학생, 미디어, 공식 등)
3. 자동으로 인터뷰 질문 생성
4. /api/simulation/interview/batch 인터페이스를 호출하여 듀얼 플랫폼에서 실제 인터뷰 진행
5. 모든 인터뷰 결과를 통합하여 다각도 분석 제공

【사용 시나리오】
- 다양한 역할의 관점에서 이벤트에 대한 견해를 파악해야 할 때 (학생은 어떻게 보는가? 미디어는? 공식 입장은?)
- 다자간 의견과 입장을 수집해야 할 때
- 시뮬레이션 에이전트의 실제 답변이 필요할 때 (OASIS 시뮬레이션 환경에서 제공)
- 보고서를 더 생생하게 만들고 "인터뷰 기록"을 포함하고 싶을 때

【반환 내용】
- 인터뷰 대상 에이전트의 신원 정보
- 각 에이전트의 Twitter와 Reddit 두 플랫폼 인터뷰 답변
- 핵심 인용문 (직접 인용 가능)
- 인터뷰 요약 및 관점 비교
...
PLAN_SYSTEM_PROMPT = """\
당신은 「미래 예측 보고서」의 작성 전문가이며, 시뮬레이션 세계에 대한 「신의 시점」을 가지고 있습니다——시뮬레이션 내 모든 에이전트의 행동, 발언, 상호작용을 통찰할 수 있습니다.

【핵심 이념】
우리는 시뮬레이션 세계를 구축하고 특정 「시뮬레이션 요구사항」을 변수로 주입했습니다. 시뮬레이션 세계의 진화 결과가 바로 미래에 발생할 수 있는 상황에 대한 예측입니다. 당신이 관찰하고 있는 것은 "실험 데이터"가 아니라 "미래의 예행연습"입니다.

【당신의 임무】
「미래 예측 보고서」를 작성하여 다음에 답하세요:
1. 우리가 설정한 조건에서 미래에 무엇이 발생했는가?
2. 각 유형의 에이전트(집단)는 어떻게 반응하고 행동했는가?
3. 이 시뮬레이션이 드러낸 주목할 만한 미래 추세와 리스크는 무엇인가?

【보고서 포지셔닝】
- ✅ 이것은 시뮬레이션 기반의 미래 예측 보고서로, "이렇게 되면 미래는 어떻게 될까"를 보여줍니다
- ✅ 예측 결과에 집중: 이벤트 전개, 집단 반응, 창발 현상, 잠재적 리스크
- ✅ 시뮬레이션 세계의 에이전트 발언과 행동이 바로 미래 인간 행동에 대한 예측입니다
- ❌ 현실 세계의 현재 상황 분석이 아닙니다
- ❌ 일반적인 여론 종합이 아닙니다

【섹션 수 제한】
- 최소 2개 섹션, 최대 5개 섹션
- 하위 섹션은 필요 없으며, 각 섹션에 완전한 내용을 직접 작성
...
【시뮬레이션으로 예측한 일부 미래 사실 샘플】
{related_facts_json}

「신의 시점」으로 이 미래 예행연습을 검토하세요:
1. 우리가 설정한 조건에서 미래는 어떤 상태를 보여주었는가?
2. 각 유형의 집단(에이전트)은 어떻게 반응하고 행동했는가?
3. 이 시뮬레이션이 드러낸 주목할 만한 미래 추세는 무엇인가?

예측 결과에 따라 가장 적합한 보고서 섹션 구조를 설계하세요.

【다시 한번 알림】보고서 섹션 수: 최소 2개, 최대 5개, 핵심 예측 발견에 집중하여 간결하게 작성하세요."""

# ── 섹션 생성 프롬프트 ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
당신은 「미래 예측 보고서」의 작성 전문가이며, 보고서의 한 섹션을 작성하고 있습니다.
...
시뮬레이션 내 에이전트의 행동과 상호작용이 바로 미래 인간 행동에 대한 예측입니다.

당신의 임무:
- 설정된 조건에서 미래에 무엇이 발생했는지 밝히기
- 각 유형의 집단(에이전트)이 어떻게 반응하고 행동했는지 예측하기
- 주목할 만한 미래 추세, 리스크, 기회를 발견하기

❌ 현실 세계의 현재 상황 분석처럼 작성하지 마세요
...
1. 【반드시 도구를 호출하여 시뮬레이션 세계를 관찰하세요】
   - 당신은 「신의 시점」으로 미래의 예행연습을 관찰하고 있습니다
   - 모든 내용은 시뮬레이션 세계에서 발생한 이벤트와 에이전트의 발언/행동에서 가져와야 합니다
   - 자신의 지식을 사용하여 보고서 내용을 작성하는 것은 금지됩니다
   - 각 섹션에서 최소 3회(최대 5회) 도구를 호출하여 미래를 대표하는 시뮬레이션 세계를 관찰하세요

2. 【반드시 에이전트의 원본 발언/행동을 인용하세요】
   - 에이전트의 발언과 행동은 미래 인간 행동에 대한 예측입니다
   - 보고서에서 인용 형식을 사용하여 이러한 예측을 보여주세요, 예:
     > "특정 유형의 집단이 이렇게 표현할 것이다: 원문 내용..."
   - 이러한 인용은 시뮬레이션 예측의 핵심 증거입니다

3. 【언어 일관성 - 인용 내용은 반드시 보고서 언어로 번역】
   - 도구가 반환하는 내용에는 영어 또는 여러 언어가 혼재된 표현이 포함될 수 있습니다
   - 시뮬레이션 요구사항과 자료 원문이 한국어인 경우, 보고서는 반드시 한국어를 사용하세요
   - 도구가 반환한 영어 또는 혼재된 언어 내용을 인용할 때, 반드시 자연스러운 한국어로 번역한 후 보고서에 작성하세요
   - 번역 시 원래 의미를 유지하고 자연스러운 표현을 보장하세요
   - 이 규칙은 본문과 인용 블록(> 형식) 내용 모두에 적용됩니다

4. 【예측 결과를 충실히 제시】
   - 보고서 내용은 시뮬레이션 세계의 미래를 대표하는 시뮬레이션 결과를 반영해야 합니다
...
- ✅ 섹션 제목은 시스템이 자동으로 추가하며, 순수 본문 내용만 작성하세요
- ✅ **굵은 글씨**, 단락 구분, 인용, 목록을 사용하여 내용을 구성하되, 제목은 사용하지 마세요

【올바른 예시】
```
이 섹션에서는 이벤트의 여론 전파 양상을 분석했습니다. 시뮬레이션 데이터에 대한 심층 분석을 통해 우리는 다음을 발견했습니다...

**초기 폭발 단계**

특정 플랫폼이 여론의 첫 번째 현장으로서 정보 발표의 핵심 기능을 담당했습니다:

> "해당 플랫폼이 68%의 초기 발신량을 차지했습니다..."

**감정 증폭 단계**

다른 플랫폼이 이벤트의 영향력을 더욱 증폭시켰습니다:
...
【도구 사용 제안 - 다양한 도구를 혼합 사용하세요, 하나만 사용하지 마세요】
- insight_forge: 심층 인사이트 분석, 자동으로 문제를 분해하고 다차원에서 사실과 관계를 검색
- panorama_search: 광각 파노라마 검색, 이벤트 전체 모습, 타임라인과 변화 과정 파악
- quick_search: 특정 구체적 정보 포인트의 빠른 검증
- interview_agents: 시뮬레이션 에이전트 인터뷰, 다양한 역할의 1인칭 관점과 실제 반응 가져오기

═══════════════════════════════════════════════════════════════
【작업 흐름】
═══════════════════════════════════════════════════════════════

매 응답에서 다음 두 가지 중 하나만 할 수 있습니다 (동시에 불가):

옵션A - 도구 호출:
당신의 사고를 출력한 후 다음 형식으로 도구를 호출하세요:
<tool_call>
{{"name": "도구 이름", "parameters": {{"매개변수 이름": "매개변수 값"}}}}
</tool_call>
시스템이 도구를 실행하고 결과를 반환합니다. 당신은 도구 반환 결과를 직접 작성할 필요도 없고 작성해서도 안 됩니다.

옵션B - 최종 내용 출력:
도구를 통해 충분한 정보를 얻었다면, "Final Answer:"로 시작하여 섹션 내용을 출력하세요.

⚠️ 엄격히 금지:
- 한 번의 응답에 도구 호출과 Final Answer를 동시에 포함하는 것
- 도구 반환 결과(Observation)를 직접 작성하는 것, 모든 도구 결과는 시스템이 주입합니다
- 한 번의 응답에 최대 하나의 도구만 호출

═══════════════════════════════════════════════════════════════
【섹션 내용 요구사항】
═══════════════════════════════════════════════════════════════

1. 내용은 반드시 도구로 검색한 시뮬레이션 데이터를 기반으로 해야 합니다
2. 원문을 많이 인용하여 시뮬레이션 효과를 보여주세요
3. Markdown 형식을 사용하되 (제목 사용은 금지):
   - **굵은 글씨**로 중요 내용을 표시 (소제목 대체)
   - 목록(- 또는 1.2.3.)으로 요점을 정리
   - 빈 줄로 서로 다른 단락을 구분
   - ❌ #, ##, ###, #### 등 어떤 제목 문법도 사용 금지
4. 【인용 형식 규범 - 반드시 독립 단락으로】
   인용은 반드시 독립된 단락이어야 하며, 앞뒤에 각각 빈 줄이 있어야 합니다. 단락 중간에 혼합할 수 없습니다:

   ✅ 올바른 형식:
   ```
   학교 측의 대응은 실질적인 내용이 부족한 것으로 여겨졌습니다.

   > "학교 측의 대응 방식은 빠르게 변화하는 소셜 미디어 환경에서 경직되고 느린 것으로 보입니다."
   ...
   ❌ 잘못된 형식:
   ```
   학교 측의 대응은 실질적인 내용이 부족한 것으로 여겨졌습니다.> "학교 측의 대응 방식은..." 이 평가는 반영합니다...
   ```
5. 다른 섹션과의 논리적 일관성 유지
6. 【중복 방지】아래의 완성된 섹션 내용을 주의 깊게 읽고 동일한 정보를 반복 기술하지 마세요
...
═══════════════════════════════════════════════════════════════
【현재 임무】섹션 작성: {section_title}
═══════════════════════════════════════════════════════════════

【중요 알림】
1. 위의 완성된 섹션을 주의 깊게 읽고 동일한 내용의 반복을 피하세요!
2. 시작 전 반드시 도구를 호출하여 시뮬레이션 데이터를 가져오세요
3. 다양한 도구를 혼합 사용하고 하나만 사용하지 마세요
4. 보고서 내용은 반드시 검색 결과에서 가져와야 하며, 자신의 지식을 사용하지 마세요

【⚠️ 형식 경고 - 반드시 준수】
- ❌ 어떤 제목도 작성하지 마세요 (#, ##, ###, #### 모두 불가)
- ❌ "{section_title}"를 시작 부분에 작성하지 마세요
- ✅ 섹션 제목은 시스템이 자동으로 추가합니다
- ✅ 본문을 직접 작성하고 **굵은 글씨**로 소절 제목을 대체하세요

시작하세요:
1. 먼저 이 섹션에 어떤 정보가 필요한지 사고(Thought)하세요
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    ...
    "도구를 더 호출하여 시뮬레이션 데이터를 가져온 후 Final Answer를 출력하세요.{unused_hint}"
)
...
REACT_TOOL_LIMIT_MSG = (
    ...
    '즉시 이미 가져온 정보를 기반으로 "Final Answer:"로 시작하여 섹션 내용을 출력하세요.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 아직 사용하지 않은 도구: {unused_list}, 다양한 도구를 사용하여 다각도 정보를 가져오는 것을 권장합니다"

REACT_FORCE_FINAL_MSG = "도구 호출 제한에 도달했습니다. 바로 Final Answer:를 출력하여 섹션 내용을 생성하세요."
...
【규칙】
1. 위의 보고서 내용을 기반으로 우선 답변하세요
2. 질문에 직접 답변하고, 장황한 사고 논술을 피하세요
3. 보고서 내용이 답변에 충분하지 않을 때만 도구를 호출하여 더 많은 데이터를 검색하세요
4. 답변은 간결하고, 명확하고, 체계적이어야 합니다
...
class ReportAgent:
    """
    Report Agent - 시뮬레이션 보고서 생성 에이전트
    ...
    def __init__(
        ...
        Args:
            ...
            graph_tools: 그래프 도구 서비스 (선택, 외부에서 GraphStorage 주입 필요)
        """
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # 심층 인터뷰 - 실제 OASIS 인터뷰 API를 호출하여 시뮬레이션 에이전트의 답변 가져오기 (듀얼 플랫폼)
            ...
            # ========== 하위 호환성을 위한 이전 도구 (내부적으로 새 도구로 리다이렉트) ==========
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        2. 베어 JSON (응답 전체 또는 한 줄이 도구 호출 JSON인 경우)
        """
        tool_calls = []
        ...
        # 형식1이 매칭되지 않았을 때만 시도, 본문의 JSON과 잘못 매칭되는 것을 방지
    ...
    def plan_outline(
        ...
        LLM을 사용하여 시뮬레이션 요구사항을 분석하고 보고서 목차 구조를 계획
        ...
        try:
            ...
            return ReportOutline(
                title="미래 예측 보고서",
                summary="시뮬레이션 예측 기반의 미래 추세 및 리스크 분석",
                sections=[
                    ReportSection(title="예측 시나리오 및 핵심 발견"),
                    ReportSection(title="집단 행동 예측 분석"),
                    ReportSection(title="추세 전망 및 리스크 안내")
                ]
            )
    ...
    def _generate_section_react(
        ...
        # 사용자 프롬프트 구성 - 각 완성된 섹션당 최대 4000자 전달
        ...
        for iteration in range(max_iterations):
            ...
            # LLM 반환이 None인지 확인 (API 예외 또는 내용이 비어있음)
            if response is None:
                logger.warning(f"섹션 {section.title} 제 {iteration + 1}차 반복: LLM이 None 반환")
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # 마지막 반복에서도 None 반환, 루프를 벗어나 강제 마무리로 진입
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                logger.warning(
                    ...
                    f"LLM이 도구 호출과 Final Answer를 동시에 출력 (제 {conflict_retries}차 충돌)"
                )
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "【형식 오류】한 번의 응답에 도구 호출과 Final Answer를 동시에 포함했습니다. 이는 허용되지 않습니다.\n"
                            "매 응답에서 다음 두 가지 중 하나만 할 수 있습니다:\n"
                            "- 도구를 하나 호출 (<tool_call> 블록 출력, Final Answer 작성하지 않기)\n"
                            "- 최종 내용 출력 ('Final Answer:'로 시작, <tool_call> 포함하지 않기)\n"
                            "다시 응답하되, 둘 중 하나만 수행하세요."
                        ),
                    })
                    continue
                else:
                    ...
                    logger.warning(
                        ...
                        "첫 번째 도구 호출만 잘라서 강제 실행으로 다운그레이드"
                    )
            ...
            # 이 내용을 최종 답변으로 직접 채택하고 더 이상 공회전하지 않음
    ...
    def generate_report(
        ...
        각 섹션 생성 완료 후 즉시 폴더에 저장하며, 전체 보고서 완료를 기다릴 필요가 없습니다.
        파일 구조:
        reports/{report_id}/
            meta.json       - 보고서 메타 정보
            outline.json    - 보고서 개요
    ...
    def chat(
        ...
        대화 중 에이전트가 자율적으로 검색 도구를 호출하여 질문에 답변
...
class ReportManager:
    ...
    def update_progress(
        ...
        프론트엔드에서 progress.json을 읽어 실시간 진행률을 가져올 수 있습니다
        """
        cls._ensure_report_folder(report_id)
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        1. 중복 제목 제거
        2. 보고서 주제목(#)과 섹션 제목(##)을 유지하고, 기타 레벨 제목(###, #### 등) 제거
        3. 불필요한 빈 줄과 구분선 정리
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # 중복 제목인지 확인 (연속 5줄 이내에 동일한 내용의 제목이 나타남)
                is_duplicate = False
