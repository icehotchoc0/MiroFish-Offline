...
설정 파일의 파라미터를 읽어 시뮬레이션 실행, 전체 과정 자동화
...
class UnicodeFormatter(logging.Formatter):
    """커스텀 포매터, Unicode 이스케이프 시퀀스를 읽기 가능한 문자로 변환"""
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            ...
            wait_for_commands: 시뮬레이션 완료 후 명령 대기 여부 (기본 True)
        """
    ...
    def _create_model(self):
        ...
        프로젝트 루트 디렉토리 .env 파일의 설정을 통합 사용 (최우선):
        - LLM_API_KEY: API 키
    ...
    async def run(self, max_rounds: int = None):
        ...
        if max_rounds is not None and max_rounds > 0:
            ...
            if total_rounds < original_rounds:
                print(f"\n라운드 수 절삭: {original_rounds} -> {total_rounds} (max_rounds={max_rounds})")
        ...
        if not os.path.exists(profile_path):
            print(f"오류: Profile 파일이 존재하지 않음: {profile_path}")
            return
        ...
        if initial_posts:
            ...
            if initial_actions:
                ...
                print(f"  {len(initial_actions)}개 초기 게시물 발행 완료")
...
def setup_signal_handlers():
    ...
    프로그램이 리소스를 정상적으로 정리할 기회 제공 (데이터베이스, 환경 종료 등)
    """
